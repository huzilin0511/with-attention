...
# 这部分代码主要在描述强化学习环境（Environment）的定义，包括环境的状态变化以及如何从当前状态转移到下一状态
# This part of the code mainly describes the definition of the reinforcement learning environment (Environment),
# including the state changes of the environment and how to move from the current state to the next state
...
class Environ:
    ...
    def renew_positions(self):
        ...
        while(i < len(self.vehicles)):
            ...
            i += 1
# 这个函数的主要作用是更新环境中每个车辆的位置。这个函数会循环遍历环境中的每一辆车，然后根据车辆的速度和时间步长来计算车辆在当前时间步内应该移动的距离。
# 然后，函数会检查车辆是否到达了一个交叉口，如果是，那么车辆有40%的概率改变方向，否则车辆会继续沿着当前的方向移动。
# 最后，函数会检查车辆是否到达了环境的边界，如果是，那么车辆会改变方向，否则车辆的位置会根据计算出来的移动距离进行更新。
# The main role of this function is to update the position of each vehicle in the environment.
# This function cycles through each car in the environment and then calculates the distance the vehicle should move in the current time step based on its speed and time step.
# The function then checks if the vehicle reaches an intersection, if so, there is a 40% chance that the vehicle changes direction, otherwise the vehicle continues to move in the current direction.
# Finally, the function checks whether the vehicle has reached the boundary of the environment, if so, then the vehicle changes direction, otherwise the position of the vehicle is updated based on the calculated moving distance.


# 这段代码中的函数主要是用于在智能交通系统中模拟和更新车辆之间（V2V）和车辆与基站之间（V2I）的无线通信信道，并计算相关的通信性能指标。
# The function in this code is mainly used to simulate and update the wireless communication channel between vehicles (V2V)
# and between vehicle and base station (V2I) in the intelligent transportation system, and to calculate the relevant communication performance indicators.
